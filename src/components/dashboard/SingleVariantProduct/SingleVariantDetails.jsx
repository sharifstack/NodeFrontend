        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-5 gap-6 flex-1">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="flex items-center gap-3 border rounded-xl p-3">
              <Avatar>
                <AvatarImage src={data?.data?.brand?.image} />
                <AvatarFallback>
                  {data?.data?.brand?.name?.charAt(0)}
                </AvatarFallback>
              </Avatar>
              <div>
          ...
          <div className="md:col-span-3 flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 bg-muted/40 p-4 rounded-xl">
                ...
                <InfoItem
                  label="Wholesale"
                  value={`৳ ${data?.data?.wholesalePrice}`}
                />
                <InfoItem label="Stock" value={data?.data?.stockQuantity} />
                <InfoItem
                  label="Status"
                  value={data?.data?.availabilityStatus}
                />
              </div>

              {/* META */}
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <InfoItem label="Category" value={data?.data?.category?.name} />
                <InfoItem
                  label="Sub Category"
                  value={data?.data?.subCategory?.name}
                />
                <InfoItem label="Unit" value={data?.data?.unit} />
                <InfoItem label="Color" value={data?.data?.color} />
              </div>

              {/* EXTRA */}
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <InfoItem label="Size" value={data?.data?.size} />
                <InfoItem
                  label="Country"
                  value={data?.data?.manufactureCountry}
                />
                <InfoItem label="Min Qty" value={data?.data?.minimumQuantity} />
                <InfoItem label="Alert Qty" value={data?.data?.alertQuantity} />
              </div>
...
export default React.memo(SingleVariantDetails);
